...
export default function MobManager({ maze, floorLevel }) {
    ...
    const { gameState, setGameState, addNotification, setBossSubtitle, updateBossStatus, fastStateRef, getLevelFromXP, getNextLevelXP, setInteractionPrompt, updateScannedTargets, triggerCrit } = useGame();
        ...
        currentMobs.forEach((mob, i) => {
            ...
            if (mob.id === 'BIT_MITE' && miteRef.current) {
                ...
                if (sentryMidRef.current) {
                    tempObject.position.set(mob.x, 2.25, mob.z); // Lowered to 2.25
                    tempObject.rotation.y = t * 0.5; // Clockwise
                    // Scale Increased by 35% (1.0 -> 1.35)
                ...
                if (sentryTopRef.current) {
                    tempObject.position.y = 2.25 + expansion; // Lowered Base
                    tempObject.rotation.y = -t * 0.8; // Counter-clockwise
                    // Stretch Y axis (X,Z=1.35, Y=2.0)
                    tempObject.scale.set(1.35, 2.0, 1.35);
                    tempObject.updateMatrix();
                ...
                // Sentry Heart: Sphere Core (Already 0.1)
                if (sentryHeartRef.current) {
                    tempObject.position.set(mob.x, 2.25, mob.z); // Lowered
